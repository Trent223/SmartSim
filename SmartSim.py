...
class MainPage:
    ...
    def Edit(self, selection, index, flag):
        ...
        self.updateBtn = tk.Button(self.currentWindow, text="Update Slider", command=lambda: UpdateSlider(self.minText, self.maxText, self.resText), bg="deep sky blue", height=1, width=14, font=("Courier", 10))
		#set the location within the window and font size
        self.updateBtn.place(relx=.18, rely=.81)

        #BUTTON: Goal Button
        self.goalBtn = tk.Button(self.currentWindow, text="Goal Calc", command=lambda: self.Solve(self.goalText, self.XText, index), bg="deep sky blue", height=1, width=9, font=("Courier", 10))
		#set the location within the window and font size
        self.goalBtn.place(relx=.32, rely=.81)
